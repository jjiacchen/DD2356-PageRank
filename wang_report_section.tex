...
required 200k-node/2M-edge graph. The remaining entries below either belong to
Minyi Zhu's OpenMP/hybrid analysis scope or to Wang's GPU-offloading comparison.
...
graph readers, integrated the MPI result pipeline, and verified the OpenMP GPU
offloading prototype against the serial baseline. A confirmed-device,
same-platform GPU comparison against the group hybrid baseline remains the
next GPU-offloading measurement.
