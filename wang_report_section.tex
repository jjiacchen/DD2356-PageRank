...
\title{DD2356 VT26 Project Report\\Pengyu Wang's Part: MPI Scaling, Optimization, and GPU Offloading}
\author{Group 26}
\date{}
...
To verify these choices directly, I added an ablation profiler that builds four
hybrid variants: \path{no_inv_static}, \path{inv_static},
\path{no_inv_dynamic}, and \path{inv_dynamic}. The formal run was conducted on
the DD2356 school-cluster CPU server with OpenMPI core binding enabled. For
each variant and layout, one warmup preceded ten correctness-checked timing
repetitions. The measured layouts were \texttt{1x1}, \texttt{1x2},
\texttt{1x4}, \texttt{1x8}, \texttt{1x16}, \texttt{2x8}, \texttt{4x4},
\texttt{8x2}, and \texttt{16x1}.

The profiler now measures both total PageRank time and sparse update-kernel
time. A diagnostic traversal outside the timed interval also counts the
incoming edges processed by each OpenMP worker; its imbalance metric is
$\max_t M_t/\overline{M_t}$. Thus this metric observes thread work directly
rather than inferring thread behavior from MPI rank imbalance. Parallel
efficiency is computed separately for each variant from its own single-worker
baseline:
\[
\begin{aligned}
S_v(P,N) &= T_v(1,1)/T_v(P,N),\\
E_v(P,N) &= S_v(P,N)/(PN).
\end{aligned}
\]
For irregular-load evidence, a controlled \texttt{skewed\_100k\_1m.csv}
input first inserts one cycle edge per node and directs 80\% of remaining
edges to the first 6250 destination nodes. This preserves a deterministic,
contiguous hot region for comparing static and dynamic schedules. Both formal
summary CSVs contain all $4\times9=36$ rows, the raw CSVs contain 360 timed
samples each, and every sample passes verification against the serial
reference.

\begin{table}[H]
\centering
\caption{Formal optimization evidence on the regular synthetic graph. PR and
update times are seconds; efficiency is relative to each variant's own
\texttt{1x1} baseline.}
...
\begin{table}[H]
\centering
\caption{Dynamic-scheduling evidence on the controlled skewed graph. The
static column uses \texttt{inv\_static}; the dynamic column uses
\texttt{inv\_dynamic}.}
...
The formal results show two different regimes. On the regular synthetic graph,
the complete optimization improves PageRank time by $1.1043\times$ at
\texttt{1x1}, $1.1702\times$ at \texttt{1x4}, and $1.1732\times$ at
\texttt{2x8}; the efficiency gain at \texttt{1x4} is $+0.0417$. It is not a
universal scaling improvement: at \texttt{1x16}, update-kernel time falls from
$0.003683$ to $0.002765$ s while end-to-end PageRank time is slightly worse,
and efficiency falls by $0.0499$. Synchronization and scheduling overhead
therefore offsets the kernel reduction at that layout. On the controlled
skewed graph, dynamic scheduling directly addresses the second bottleneck:
at \texttt{1x16}, imbalance falls from $11.7988$ to $1.2415$ and
update-kernel time falls from $0.017573$ to $0.003117$ s. This is a measured
thread-load result, not a claim inferred from MPI-rank work.
...
\caption{Before/after parallel efficiency on the regular graph (left), and
static versus dynamic thread imbalance on the skewed stress graph (right).}
...
The formal school-cluster CPU optimization experiment closes Wang's
profiling-guided optimization evidence with update-kernel, thread-imbalance,
and parallel-efficiency measurements. The remaining entries below belong to
Minyi Zhu's OpenMP/hybrid analysis scope.
...
Formal CPU ablation: two inputs, four variants, nine layouts, ten repeats; thread imbalance and efficiency included &
Complete for Wang's optimization scope \\
\bottomrule
\end{tabular}
\end{table}
...
OpenMP GPU offloading. Wang also completed the formal CPU ablation evidence for
reciprocal out-degree caching and dynamic scheduling, including before/after
parallel efficiency and controlled thread-imbalance measurements. The final
GPU evidence confirms execution on an NVIDIA H100 MIG device, demonstrates the
persistent-device-data optimization against the naive target baseline, and
compares it with the group Hybrid \texttt{4x2} baseline on the same server and
graph.
