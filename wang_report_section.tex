...
\title{DD2356 VT26 Project Report\\Pengyu Wang's Part: MPI, Hybrid MPI+OpenMP, and GPU Offloading}
\author{Group 26}
\date{}
...
The profiling script supports fixed-core experiments with configurations
...
The GPU comparison path is implemented with OpenMP target offloading. The
dangling-node reduction and sparse PageRank update are expressed with
\texttt{target teams distribute parallel for}; the convergence norm remains on
the CPU in the current prototype. The implementation maps the PageRank vector,
CSR arrays, and precomputed inverse out-degree array to the device for each
offloaded kernel. This is intentionally a correctness-first prototype, useful
for comparing the cost of offloading against the best CPU paths before adding
more aggressive persistent device-data regions.
...
and the Dardel scaling experiments passed for all measured $P=1,2,4,8,16$
cases. On the school cluster, the MPI strong-scaling runs for both
...
MPI & Dardel scaling datasets & 1, 2, 4, 8, 16 ranks & PASS, tol. $10^{-6}$ \\
...
OpenMP target GPU & 9 course datasets & local target-offload prototype & PASS vs. serial \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:wang-polblogs-strong} reports the Dardel strong-scaling result
for the course \texttt{polblogs.csv} graph. The graph is small
...
\begin{table}[H]
\centering
\caption{MPI strong scaling on Dardel for \texttt{polblogs.csv} (directed).}
\label{tab:wang-polblogs-strong}
\begin{tabular}{rrrrrrr}
\toprule
Ranks & Runs & PR time (s) & Comm frac. & Speedup & Efficiency & Imbalance \\
\midrule
1  & 5 & 0.002560 & 0.0430 & 1.000000 & 1.000000 & 1.000 \\
2  & 5 & 7.836485 & 0.9997 & 0.000327 & 0.000163 & 1.887 \\
4  & 5 & 8.291886 & 0.9998 & 0.000309 & 0.000077 & 3.129 \\
8  & 5 & 11.579997 & 0.9999 & 0.000221 & 0.000028 & 4.412 \\
16 & 5 & 14.117646 & 1.0000 & 0.000181 & 0.000011 & 4.536 \\
\bottomrule
\end{tabular}
\end{table}
...
(max/average in-edge imbalance around $1.05$), but Table~\ref{tab:wang-synth-strong}
still shows negative scaling after one rank. This confirms that the bottleneck
is dominated by collective communication rather than by local load imbalance.

\begin{table}[H]
\centering
\caption{MPI strong scaling on Dardel for \texttt{synthetic\_100k\_1m.csv} (directed).}
\label{tab:wang-synth-strong}
\begin{tabular}{rrrrrrr}
\toprule
Ranks & Runs & PR time (s) & Comm frac. & Speedup & Efficiency & Imbalance \\
\midrule
1  & 5 & 0.036353 & 0.0127 & 1.000000 & 1.000000 & 1.000 \\
2  & 5 & 8.682688 & 0.9961 & 0.004187 & 0.002093 & 1.035 \\
4  & 5 & 7.217915 & 0.9976 & 0.005037 & 0.001259 & 1.044 \\
8  & 5 & 10.115917 & 0.9989 & 0.003594 & 0.000449 & 1.047 \\
16 & 5 & 16.303321 & 0.9996 & 0.002230 & 0.000139 & 1.051 \\
\bottomrule
\end{tabular}
\end{table}

The communication fraction rises to approximately $0.996$--$0.999$ for
$P>1$ on the synthetic graph. In the timing breakdown, most of this time is
spent in the full-vector \texttt{MPI\_Allgatherv} and the synchronization waits
inside the scalar reductions. Therefore, this implementation is correct and
useful as a distributed-memory baseline, but it does not provide scalable
performance without reducing the amount of vector communication per iteration.

\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{results/figures/mpi_speedup.png}
\includegraphics[width=0.48\textwidth]{results/figures/mpi_efficiency.png}
\caption{MPI speedup and parallel efficiency from the generated result figures.}
\label{fig:wang-mpi-speedup-efficiency}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{results/figures/mpi_runtime_breakdown.png}
\includegraphics[width=0.48\textwidth]{results/figures/mpi_comm_fraction.png}
\caption{MPI runtime breakdown and communication fraction.}
\label{fig:wang-mpi-runtime-comm}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{results/figures/mpi_workload_balance.png}
\caption{MPI workload imbalance measured as max in-edges divided by average in-edges.}
...
\begin{table}[H]
\centering
\caption{MPI weak scaling on Dardel with 125k edges per rank.}
...
\midrule
\texttt{weak\_1rank\_12500\_125000}    & 1  & 12500  & 125000  & 0.003857  & 0.0338 & 1.000000000 & PASS \\
\texttt{weak\_2rank\_25000\_250000}    & 2  & 25000  & 250000  & 2.004018  & 0.9978 & 0.001924534 & PASS \\
\texttt{weak\_4rank\_50000\_500000}    & 4  & 50000  & 500000  & 2.101621  & 0.9975 & 0.001835155 & PASS \\
\texttt{weak\_8rank\_100000\_1000000}  & 8  & 100000 & 1000000 & 9.919046  & 0.9987 & 0.000388828 & PASS \\
\texttt{weak\_16rank\_200000\_2000000} & 16 & 200000 & 2000000 & 23.825432 & 0.9992 & 0.000161877 & PASS \\
\bottomrule
\end{tabular}
\end{table}
...
weak-scaling run, \texttt{MPI\_Allgatherv} alone averages $21.812665$ s out of
$23.825432$ s PageRank time, so increasing local work per rank is not enough to
hide the global communication cost.

\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{results/figures/dardel_weak/mpi_weak_efficiency.png}
\includegraphics[width=0.48\textwidth]{results/figures/dardel_weak/mpi_comm_fraction.png}
\caption{Dardel weak-scaling efficiency and communication fraction.}
...
The GPU offloading prototype was correct on all course datasets, but it was not
faster on the measured local \texttt{polblogs.csv} case. The local serial PR
time was $0.001943$ s, while the OpenMP target version took $0.053566$ s. The
best school-cluster hybrid result for \texttt{polblogs.csv} was also much
lower at $0.0029068$ s. This is not an apples-to-apples hardware comparison,
but it shows that the current graph is too small for GPU offloading to amortize
target launch and data mapping overheads, especially because the prototype maps
arrays for each target region instead of keeping device data resident across the
full iteration loop.

\begin{table}[H]
\centering
\caption{Local GPU comparison on \texttt{polblogs.csv} (directed).}
\label{tab:wang-gpu-comparison}
\begin{tabular}{llll}
\toprule
Variant & Configuration & PR time (s) & Correctness \\
\midrule
Serial & 1 thread & 0.001943 & reference \\
OpenMP target GPU & target offload prototype & 0.053566 & PASS \\
\bottomrule
\end{tabular}
\end{table}
...
The new weak-scaling measurements close the previously missing 16-rank cases
on both Dardel and the school cluster. However, the current evidence must not
be interpreted as satisfying every A-level measurement requirement. In
particular, the Dardel MPI measurements in this section were collected in a
single-node allocation, whereas the requested criterion specifies multi-node
Dardel strong and weak scaling.
...
Verified strong and weak CSVs for 1--16 ranks on one shared node &
Repeat scaling with ranks placed across at least two Dardel nodes \\
Hybrid fixed-core search on school cluster &
Verified \texttt{1x16} through \texttt{16x1} sweeps &
Repeat for additional total core budgets, not only $P\times N=16$ \\
OpenMP speedup on three systems &
Correctness and local timing evidence &
Provide scaling measurements on Colab, KTH, and Dardel \\
GPU offload comparison with Hybrid &
Correct local offload prototype and separate school-cluster Hybrid timing &
Obtain a same-platform GPU-vs-Hybrid timing comparison with confirmed device execution \\
\bottomrule
\end{tabular}
\end{table}
...
analyzed the Dardel and school-cluster MPI strong-scaling experiments, added
the 16-rank weak-scaling workflow with dynamically sized graph readers, built
and evaluated the hybrid MPI+OpenMP fixed-core profiling workflow, integrated
the cluster result pipeline, and verified/compared the OpenMP GPU offloading
prototype against the serial baseline.
