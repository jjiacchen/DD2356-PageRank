...
int main(int argc, char **argv) {
    ...
    if (argc < 4) {
        if (rank == 0) {
            printf("Usage: %s <csv_file> <directed|undirected> <threads> [damping=0.85] [tol=1e-10] [max_iter=1000] [output=pagerank_hybrid_output.txt]\n", argv[0]);
        }
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        printf("=== DD2356 Hybrid MPI+OpenMP PageRank ===\n");
        printf("File      : %s\n", filename);
